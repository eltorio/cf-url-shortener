...
const cookieKey = "auth0_cookie_bc103fcfae27d120213421396497945b72f55401";
const auth0 = {
    scheme: 'https://',
    domain: AUTH0_DOMAIN,
...
const exchangeCode = async code => {
  ...
  return persistAuth(
    await fetch(auth0.scheme + AUTH0_DOMAIN + '/oauth/token', {
      method: 'POST',
...
const validateToken = token => {
  try {
    ...
    if (iss !== auth0.scheme + AUTH0_DOMAIN) {
      throw new Error(
        `Token iss value (${iss}) doesn't match AUTH0_DOMAIN (${auth0.scheme + AUTH0_DOMAIN})`,
      )
    }
...
const redirectUrl = state =>
  `${auth0.scheme +auth0.domain}/authorize?response_type=code&client_id=${
    auth0.clientId
  }&redirect_uri=${
    auth0.callbackUrl
